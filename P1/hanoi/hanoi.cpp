...
int main(int argc, char **argv)
{
  ...
  if (argc!=2){
    cout<<"\nError, introduzca el número de discos"<<endl;
    exit(1);
  }
  ...
  while (M <= 0)
    ...
    tantes = clock(); //Obtenemos el tiempo
    hanoi(M, 1, 2);
    tdespues = clock();
    cout << M<< " " << ((double)(tdespues-tantes))/CLOCKS_PER_SEC //Obtenemos el tiempo transcurrido
         << endl;
